...
const Shelter = (props) => {
  ...
  useEffect(()=> {
    if(sessionStorage.getItem('preID')){
      ...
      axios.post(process.env.NODE_ENV!=='production'?'http://localhost:4000/api/user/getshelteronid':'http://localhost:4000/api/user/getshelteronid',{token:sessionStorage.getItem('token'), id:sessionStorage.getItem('preID')}).then(res=> {
        setShelter(
          res.data.center
        )
...
''}
        ...
        <div data-aos="fade-up" data-aos-once='true' className="max-w-2xl mx-auto pt-10 pb-16 px-4 sm:px-6 lg:max-w-7xl lg:pt-16 lg:pb-24 lg:px-8 lg:grid lg:grid-cols-3 lg:grid-rows-[auto,auto,1fr] lg:gap-x-8">
          <div className="lg:col-span-2 lg:border-r lg:border-gray-200 lg:pr-8">
            <h1 className="text-2xl font-extrabold tracking-tight text-gray-900 sm:text-3xl">{shelter?shelter.name:''}</h1>
          </div>
          ...
          <h3 class = 'text-2xl font-bold text-center mb-3'>Contact Details</h3>
          <h3 class = 'text-sm mt-5 text-center'>Email Address: {shelter?shelter.email:''}</h3>
          <h3 class = 'text-sm text-center'>Mobile/Landline Number: {shelter?shelter.contact:''}</h3>
          ...
          <div className="py-10 lg:pt-6 lg:pb-16 lg:col-start-1 lg:col-span-2 lg:border-r lg:border-gray-200 lg:pr-8">
            ...
            <div>
              ...
              <div className="space-y-6">
                <p className="text-base text-gray-900">{shelter? shelter.desc.replace('/n','&#10;'):''}</p>
              </div>
            </div>

            <div className="mt-10">
              <h3 className="text-sm font-medium text-gray-900">Steps to Adopt</h3>
            ...
            <div className="mt-10">
              ...
              <button
                ...
                className="mt-14 -mb-10 w-1/3 ring-4 mx-auto bg-white border border-transparent rounded-md py-3 px-8 flex items-center justify-center text-base font-medium text-black hover:bg-amber-600 hover:text-white focus:outline-none ring-offset-2 ring-amber-600"
              >
                Apply to Adopt
              </button>
            </div>
